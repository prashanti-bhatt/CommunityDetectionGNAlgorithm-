...
output.write("Edge Centrality\t\tRemoved edge\n")
...
def runGirvanNewman(G, Orig_deg, m_):
    ...
    while True:    
        ...
        if Q > BestQ:
            fo = open("Connected_component_list.txt", "w")
            BestQ = Q
            Bestcomps = nx.connected_components(G)    #Best Split
            print("Components:", Bestcomps)
            fo.write("Modularity of decomposed G: {0}\n".format(Q))
            communities = list(Bestcomps)
            for comnty in communities:
                cm_data = list(comnty)
                nodes = list(cm_data)
                fo.write("{0}\n".format(comnty))
                fo.write("{0}\n".format(G.edges(nodes)))
            fo.close()
